...
class EditInvoice extends StatefulWidget {
  ...
  EditInvoice(this.token, this.invoiceId,this.clientId, {Key? key}) : super(key: key);

  @override
  State<EditInvoice> createState() => _AddInvoiceState();
}

class _AddInvoiceState extends State<EditInvoice> {
  ...
  getData() async {
    ...
    invDetails = await HttpService.invoiceCommonDetails(widget.token, widget.clientId);
    //invEditDetails = await HttpService.invoiceEditDetails(widget.token, widget.clientId);
    if (invDetails != null) {
      billingName.text =
                                                                ...
                                                                TextField(
                                                                  ...
                                                                  onChanged:
                                                                    ...
                                                                    setState(
                                                                            () {
                                                                          filteredItems =
                                                                              items.where((item) => item.productName!.toLowerCase().contains(value.toLowerCase())).toList();
                                                                        });
                                                                  },
                                                                  decoration:
                                                                  const InputDecoration(
                                                ...
                                                totalTaxAmount =
                                                    totalTaxAmount +
                                                        double.parse(
                                                            productTaxAmount
                                                                .text);
                                                allTotal = subTotal + double.parse(shippingCharge.text==''?'0':shippingCharge.text) -
                                                    double.parse(discount.text==''?'0':discount.text);
                                                productName =
                                                "Choose Product";
                                                productId = "";
                                                productDescription
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  onTap: () {
                                    ...
                                    allTotal = subTotal +
                                        double.parse(
                                            shippingCharge.text==''?'0':shippingCharge.text) -
                                        double.parse(discount.text==''?'0':discount.text);
                    ...
                    onTap: ()  async {
                      if(products.isEmpty){
                        ...
                        var body = FormData.fromMap({
                          "token": widget.token,
                          'invoice_number': invDetails!.data!.invoiceNumber,
                          'invoice_date': DateFormat("dd-MM-yyyy").format(DateTime.parse(fromdate.toString())),
                        ...
                        AddInvoiceModel inv = await HttpService.addInvoice(body);
                        if (inv.data == true) {
                          Common.toastMessaage(
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                  builder: (context) => EditInvoice(
                                      widget.token, widget.invoiceId,widget.clientId)),
                            );
                          }
                        }
                        else {
  ...
  void calculateTaxAmt() {
    setState(() {
      productTaxAmount.text = (int.parse(productRate.text) *
          int.parse(productQty.text) *
          int.parse(productTaxPercent.text) /
          100)
          .toString();
      if (productTaxAmount.text.isNotEmpty) {
        productTotalAmount.text =
            (int.parse(productRate.text) * int.parse(productQty.text) +
