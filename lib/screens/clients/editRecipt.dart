...
import 'package:kdc_chitty_mainnew/models/clients/editReceiptDetailsModel.dart';
...
class _EditReceiptState extends State<EditReceipt> {
  ...
  getData() async {
    ...
    receiptDetails = await HttpService.editReceiptDetails(widget.token,widget.receiptId);
    if (receiptDetails != null) {
      setState(() {
        collectedBy=receiptDetails!.data!.collectedBy;
            ...
            children: [
              ...
              InkWell(
                onTap: () async {
                  ...
                  else if(int.parse(payAmount.text)>int.parse(receiptDetails!.data!.checkAmount.toString()))
                  {
                    Common.toastMessaage(
                        'Maximum Amount to pay is ${receiptDetails!.data!.checkAmount}',
                        Colors.red);
                  }
                  else{
                    if (context.mounted) {
                          ...
                          object.message, Colors.green);
                      {
                        if(mounted){
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => EditReceipt(widget.token,widget.receiptId)),
                          );
                        }
